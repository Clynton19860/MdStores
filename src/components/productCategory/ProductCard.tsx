...
import { HeartIcon } from "@/components/productDetail/Icons";
...
export default function ProductCard({
  ...
  const itemInWishlist = isInWishlist(id);
        ...
        <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
          <div className="flex justify-center gap-2">
            ...
            <button 
              ...
              className={`w-10 h-10 ${itemInWishlist ? 'bg-[#FFC0CB]' : 'bg-white'} rounded-full flex items-center justify-center hover:bg-[#FFC0CB] transition-colors`}
              aria-label="Add to wishlist"
            >
              <HeartIcon className={`h-5 w-5 ${itemInWishlist ? 'text-white' : 'text-gray-700'}`} filled={itemInWishlist} />
            </button>
          </div>
        </div>
      </div>
